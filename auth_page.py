            ...
            with col1:
                if st.button('Sign in', key='auth_signin'):
                    ...
                    if ok:
                        ...
                        # If billing hasn't blocked the session, ensure the app navigates to Home
                        try:
                            if not st.session_state.get('billing_block'):
                                st.session_state['current_page'] = 'home'
                                st.session_state['show_home_header'] = True
                                st.session_state['force_delegate_home'] = True
                        except Exception:
                            # best-effort: set minimal navigation keys
                            try:
                                st.session_state['current_page'] = 'home'
                                st.session_state['show_home_header'] = True
                            except Exception:
                        ...
                        # Ensure the app delegates to Home on next rerun. Use the
                        # auth helper's safe rerun function which works across
                        # Streamlit versions.
                        try:
                            st.session_state['force_delegate_home'] = True
                        except Exception:
            ...
            if st.button('Register'):
                if not runame or not rpass:
                    ...
                    if ok:
                        ...
                        # After registration, navigate to Home unless billing blocks the session
                        try:
                            if not st.session_state.get('billing_block'):
                                st.session_state['current_page'] = 'home'
                                st.session_state['show_home_header'] = True
