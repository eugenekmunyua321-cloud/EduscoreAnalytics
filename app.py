...
# Define persistent storage path (per-school when signed in)
BASE_STORAGE = os.path.join(os.path.dirname(__file__), 'saved_exams_storage')
def get_storage_dir():
    """Return the storage directory path. If a school is signed in, use a per-school subfolder."""
    try:
        sid = auth.get_current_school_id()
        if sid:
            path = os.path.join(BASE_STORAGE, sid)
...
def save_exam_to_disk(exam_id, exam_metadata, exam_data, exam_raw_data, exam_config):
    ...
    try:
        ...
        # Save metadata to main JSON file
        all_metadata = {}
        ...
        # If Firebase is configured and initialized, upload metadata and files
        try:
            if fb is not None and getattr(fb, 'is_initialized', lambda: False)():
                try:
                    ...
                    for fname in ('data.pkl', 'raw_data.pkl', 'config.json'):
                        lp = os.path.join(exam_dir, fname)
            ...
            # best-effort upload; ignore any failures here so local save still succeeds
            pass
...
def load_config():
    ...
    try:
        ...
        if os.path.exists(per_account_cfg):
            try:
