    ...
    if send:
        if not p_msg or not p_phone:
            ...
            try:
                ...
                # proceed; open() will raise if impossible
                pass

            try:
                # re-load existing messages in case they changed since page load
                if msgs_path.exists():
                    try:
                ...
                # update in-memory list so the 'Your recent messages' list shows immediately
                # (no rerun required)
                # existing_msgs already updated above
