...
try:
    ...
    STORAGE = os.path.join(ROOT, 'saved_exams_storage')
META = os.path.join(STORAGE, 'exams_metadata.json')
...
def load_metadata():
    if not os.path.exists(META):
        print('No metadata file found at', META)
        return {}
...
def build_combined_df_for_exams(exam_ids):
    dfs = []
    for eid in exam_ids:
        exam_dir = os.path.join(STORAGE, eid)
        raw = os.path.join(exam_dir, 'raw_data.pkl')
        data = os.path.join(exam_dir, 'data.pkl')
        dfi = None
        try:
            if os.path.exists(raw):
                dfi = pd.read_pickle(raw)
            elif os.path.exists(data):
                dfi = pd.read_pickle(data)
