...
# Use central storage helper which returns per-school storage when signed in
STORAGE_DIR = get_storage_dir()
EXAMS_META = os.path.join(STORAGE_DIR, 'exams_metadata.json')
ASSIGNMENTS_PATH = os.path.join(STORAGE_DIR, 'ta_assignments.json')
TEACHERS_PATH = os.path.join(STORAGE_DIR, 'ta_teachers.json')
UI_STATE_PATH = os.path.join(STORAGE_DIR, 'ta_ui_state.json')


def list_saved_exams():
    if not os.path.exists(EXAMS_META):
        return []
    try:
        with open(EXAMS_META, 'r', encoding='utf-8') as f:
            data = json.load(f)
...
def load_exam_dataframe(exam_id):
    exam_dir = os.path.join(STORAGE_DIR, str(exam_id))
    if not os.path.isdir(exam_dir):
        return None
    p1 = os.path.join(exam_dir, 'data.pkl')
    p2 = os.path.join(exam_dir, 'raw_data.pkl')
    try:
        if os.path.exists(p1):
            return pd.read_pickle(p1)
...
def load_teachers():
    ...
    try:
        loaded = json.load(open(TEACHERS_PATH, 'r', encoding='utf-8')) if os.path.exists(TEACHERS_PATH) else None
...
def save_teachers(d):
    try:
        os.makedirs(STORAGE_DIR, exist_ok=True)
        # ensure we always write the normalized shape
