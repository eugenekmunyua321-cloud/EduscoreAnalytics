...
if not all_metadata:
    ...
    # Pre-load all student names and admission numbers for autocomplete
    all_students = {}  # key: (name, adm_no), value: list of exam_ids where found
    ...
    with scol1:
        ...
        selected_name = st.selectbox(
            ...
            index=0,
            help="Start typing to see matching student names"
        )
    
    with scol2:
        # Admission number search
        selected_adm = st.selectbox(
            "🔢 Search by Admission Number",
            options=[""] + student_adm_nos,
            index=0,
            help="Select student by admission number"
        )
    ...
    # Normalize class names for filter
    normalized_classes = {}  # original -> normalized
    for e in exams_list:
        if e.get('class_name'):
            normalized_classes[e.get('class_name')] = normalize_class_name(e.get('class_name'))
    
    # Get unique normalized classes
    ...
    if search_term:
        ...
        st.info("👆 Select a student name or admission number from the dropdowns above to view their complete exam history")
