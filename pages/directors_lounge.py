...
try:
    msgs_dir = Path(STORAGE_DIR) / 'messages'
    ...
    def save_archive(items):
        try:
            tmp = Path(archive_path).with_suffix('.tmp')
            with open(tmp, 'w', encoding='utf-8') as af:
                json.dump(items, af, ensure_ascii=False, indent=2)
            try:
...
def load_exam_from_disk_local(exam_id):
    try:
        ...
        exam_data = pd.read_pickle(data_path) if os.path.exists(data_path) else None
        exam_raw = pd.read_pickle(raw_path) if os.path.exists(raw_path) else None
        cfg = {}
...
def delete_exam_from_disk_local(exam_id):
    try:
        ...
        try:
            if os.path.exists(META_PATH):
                with open(META_PATH, 'r', encoding='utf-8') as f:
                    all_md = json.load(f) or {}
            else:
                all_md = {}
            all_md.pop(exam_id, None)
            with open(META_PATH, 'w', encoding='utf-8') as f:
                json.dump(all_md, f, indent=2, ensure_ascii=False)
        except Exception:
            pass
        return True
    except Exception:
...
def compute_teacher_ranking_for_exam(eid, df):
    # load assignments mapping
    assignments_path = os.path.join(STORAGE_DIR, 'ta_assignments.json')
    try:
...
# Load assignments from the same source used by Teacher Analysis
assignments_path = os.path.join(STORAGE_DIR, 'ta_assignments.json')
assignments = {}
if os.path.exists(assignments_path):
    try:
...
if not selected_ids_local:
    ...
    ta_teachers_path = os.path.join(STORAGE_DIR, 'ta_teachers.json')
    class_map_local = {}
    try:
        if os.path.exists(ta_teachers_path):
            with open(ta_teachers_path, 'r', encoding='utf-8') as tf:
                tdata = json.load(tf) or {}
                class_map_local = tdata.get('class_map', {}) if isinstance(tdata, dict) else {}
    except Exception:
        class_map_local = {}

    # caching / persistence: compute a cache key based on final_ids, filters and assignments mtime
    try:
        assign_mtime = os.path.getmtime(assignments_path) if os.path.exists(assignments_path) else 0
    except Exception:
        assign_mtime = 0
    ...
    # if a cached snapshot exists, load it (persistence across refreshes)
    if os.path.exists(cache_file):
        try:
            df_teach = pd.read_csv(cache_file)
        except Exception:
            df_teach = None
    else:
