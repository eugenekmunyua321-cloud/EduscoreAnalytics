...
PERSISTENT_CONFIG_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'app_persistent_config.json')

def load_persistent_config():
    try:
        if os.path.exists(PERSISTENT_CONFIG_FILE):
...
def save_exam_to_disk(exam_id, exam_metadata, exam_data, exam_raw_data, exam_config):
    ...
    try:
        ...
        # If Firebase helper is available, try to upload the metadata and files
        try:
            try:
            ...
            if _fb is not None and getattr(_fb, 'is_initialized', lambda: False)():
                ...
                try:
                    _fb.upload_blob(METADATA_FILE, f"{base_blob_prefix}/exams_metadata.json")
                except Exception:
                    pass
                for fname in ('data.pkl', 'raw_data.pkl', 'config.json'):
                    ...
                    if os.path.exists(lp):
                        try:
                            _fb.upload_blob(lp, f"{base_blob_prefix}/{exam_id}/{fname}")
                        except Exception:
                            pass
        except Exception:
            pass
...
def load_exam_from_disk(exam_id):
    ...
    try:
        ...
        exam_raw_data = pd.read_pickle(raw_data_path) if os.path.exists(raw_data_path) else None
