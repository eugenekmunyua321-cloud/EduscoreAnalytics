...
STORAGE_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'saved_exams_storage')
METADATA_FILE = os.path.join(STORAGE_DIR, 'exams_metadata.json')
REPORT_SETTINGS_FILE = os.path.join(STORAGE_DIR, 'report_card_settings.json')
...
if len(st.session_state.saved_exams) == 0:
    ...
    if logo_file:
        logo_path = os.path.join(STORAGE_DIR, 'temp_logo.png')
        with open(logo_path, 'wb') as f:
            f.write(logo_file.getbuffer())
    
    if logo2_file:
        logo2_path = os.path.join(STORAGE_DIR, 'temp_logo2.png')
        with open(logo2_path, 'wb') as f:
            f.write(logo2_file.getbuffer())
    
    if stamp_file:
        stamp_path = os.path.join(STORAGE_DIR, 'temp_stamp.png')
                ...
                try:
                    if path and os.path.exists(path):
                        ...
                        if st.button(f"Remove {label}", key=remove_key):
                            # Clear both the active variable and saved settings so it won't come back on rerun
                            try:
